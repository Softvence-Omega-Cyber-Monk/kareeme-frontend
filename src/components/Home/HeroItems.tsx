  ...
  return (
    ...
    >
      ...
      {/* ===========================
          ★ SMALL TOP TAG ★
          =========================== */}
      {/* Top Tag */} <motion.div initial={{ opacity: 0, y: -20 }}
   ...
   sm:w-[260px] md:w-[320px] lg:w-[396px] h-[40px] sm:h-[44px] md:h-[48px] lg:h-[52px] mx-auto px-3" > <img src={smallLogoIcon} alt="Small Logo" className="h-3 w-3 sm:h-4 sm:w-4
    md:h-5 md:w-5 lg:h-5 lg:w-5 object-contain logo-spin" /> <p className="text-xs sm:text-sm md:text-base lg:text-sm text-white font-light tracking-widest uppercase">
          DISTRIBUTION | BRANDING | STRATEGY </p>
      </motion.div>
      ...
      <h1 className="font-extrabold leading-snug sm:leading-tight md:leading-tight tracking-wide mb-6 sm:mb-8 md:mb-8">
        <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl flex justify-center flex-wrap gap-1">
          WELCOME TO{" "} {title1.map((char, idx) =>
          (<motion.span key={idx} initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1, type: "spring", stiffness: 300, damping: 20 }}
            className="text-3xl sm:text-5xl md:text-7xl font-bold tracking-widest" style={{ color: "transparent", WebkitTextStroke: "2px #f0fdf4" }} > {char} </motion.span>))}
        </span>
        <br className="block md:hidden" />
        <br />
        <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl flex justify-center flex-wrap gap-1">
          {title2.map((char, idx) => (<motion.span key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{
            delay: idx * 0.08 + 0.5, type:
              "spring", stiffness: 300, damping: 20
          }} className="text-3xl sm:text-5xl md:text-7xl font-bold tracking-widest" style={{
            color: "transparent", WebkitTextStroke:
              "2px #f0fdf4"
          }} > {char}
          </motion.span>))}
          {" "}WHERE DREM SHIFT THE </span>
        <br />
        <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl"> CULTURE!! </span>
      </h1>
